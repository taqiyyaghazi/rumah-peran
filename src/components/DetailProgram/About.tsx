...
import { IBenefit, IInstructor, ISchedule } from '@/types';
...
const About = ({
  ...
  return (
    <section className="container px-4 flex max-lg:flex-col-reverse max-lg:items-center mx-auto gap-12 -mt-10 z-10 relative overflow-hidden">
      <div className="w-full">
            ...
            <div className="grid md:grid-cols-2 grid-flow-row gap-4">
              {instructors.map(({ name, title, image }, id) => (
                <div key={id} className="flex items-center gap-x-4 w-full">
                  <div className="bg-soft-cream relative outline outline-2 outline-bright-yellow rounded-full w-16 aspect-square mb-4 flex-none">
                    <div className="absolute w-16 aspect-[3/4] rounded-full overflow-hidden bottom-0 left-0">
                      <Image
                        src={image}
                        alt={name}
                        fill
        ...
        {!!benefits.length && (
          <div>
            ...
            >
              Benefit Mengikuti Kelas
            </p>
            <div className="grid md:grid-cols-3 gap-4 mb-16">
              {benefits.map(({ title, icon }, id) => (
                <div
                  key={id}
        ...
        {!!materials.length && (
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-16 border border-[#E4E4E4] px-8 py-4 rounded-xl">
              {materials.map((material, id) => (
                <div key={id} className="flex items-center gap-4">
                  <div className="w-4 bg-[#EC9B3D] aspect-square rounded-full flex-none" />
                  <p
                    className={`${gilroy.className} text-soft-black max-sm:text-sm font-bold`}
                  >
                    {material}
                  </p>
                </div>
        ...
        {!!schedules.length && (
          <div className="mb-16">
            ...
            <div>
              {schedules.map(({ batch, startDate, endDate }, id) => (
                <div
                  key={id}
                  className={`${
          ...
          <div>
            {!!nearestSchedule && (
              <div className="flex items-center gap-x-2 mb-4">
                ...
                >
                  {formatDateRange(
                    nearestSchedule.startDate,
                    nearestSchedule.endDate
                  )}
                </p>
              </div>
            )}
